...
# TODO: Change MyPowerPlant to NonconvexRollingHorizonFlow
class MyPowerPlant(network.Transformer):
    ...
    * To use this object the connected buses need to be of the type
     ...
     * :py:class:`~oemof.solph.custom.MyPowerPlantBlock`
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.T_offl_min_hs = kwargs.get('t_offl_min_hs')
        self._helper_variables = None
        self.tau = 1
        # Sequenced values
        self.interval_time_idx = kwargs.get('interval_time_idx')
        self.T_int = len(self.interval_time_idx)
        self.operation_state = pd.Series(data=0,
                                         index=self.interval_time_idx,
                                         dtype=np.int16)

    def _calculate_helper_variables(self, T):
        helper_variables = {'sum_start_ini': 0, 'T_ini': 0, 'Z_ini_ws': 0,
                            'Z_ini_cs': 0, 'T_offl_possible_ws': 0,
                            'T_offl_possible_cs': 0}
        if T > 0:
            last_state = self.operation_state[T-1]
            for x, status in enumerate(reversed(self.operation_state[:T])):
                if (status == last_state):
                    ...
                    break
            # T_ini
            if (self.operation_state[T-1] == 0) and (helper_variables['sum_start_ini'] < self.T_offl_min_hs/self.tau):
                helper_variables['T_ini'] = min(self.T_int, self.T_offl_min_hs/self.tau-helper_variables['sum_start_ini'])
            elif (self.operation_state[T-1] == 0) and\
            (self.T_offl_min_ws/self.tau > helper_variables['sum_start_ini']) and (helper_variables['sum_start_ini'] >= int(self.T_offl_th_ws/self.tau)):
                ...
                0
            # Z ini warm start
            if (self.operation_state[T-1] == 0) and (helper_variables['sum_start_ini'] < int(self.T_offl_th_ws/self.tau)) and\
            ...
            # Z ini cold start
            if (self.operation_state[T-1] == 0) and (helper_variables['sum_start_ini'] < int(self.T_offl_th_cs/self.tau)) and\
    ...
    def constraint_group(self):
        return MyPowerPlantBlock

class MyPowerPlantBlock(SimpleBlock):
    ...
    **The following variable are created:**
    ...
    def _create(self, group=None):
        """ Creates the linear constraint for the class:`MyPowerPlantBlock`
        block.

        Parameters
        ----------
        group : list
            List containing rolling_horizon power plant objects.
            e.g. groups=[mypowerplant1, mypowerplant2,...])
        """
        # TODO:
        # Variables to fill in:
        # self.T_int, T_offl_min_hs,
        #
        if group is None:
            return None
        ...
        # create my power plant set
        self.MYPOWERPLANTS = Set(initialize=[n for n in group])

        # create hot start rule
        def hot_start(block, n, t):
            if t > 1:
                ...
                return (m.generator_status[u,t-1]-m.generator_status[u,t])*\
                    T_offl_hs[u,t-1] <= (pe.quicksum((1-m.generator_status[u,h]) for h in range(t,t+T_offl_hs[u,t-1]-1+1)))
            else:
                return Constraint.Skip
        self.hot_start = Constraint(self.MYPOWERPLANTS, m.TIMESTEPS,
                                    rule=hot_start)
